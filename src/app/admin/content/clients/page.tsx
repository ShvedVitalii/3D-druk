...
export default function EditClients() {
  ...
  const [data, setData] = useState<Data>({
    description: 'Нам довіряють провідні компанії',
    items: [
      { id: '1', name: 'Компанія 1', logo: '/images/clients/logo1.png', order: 1 },
      { id: '2', name: 'Компанія 2', logo: '/images/clients/logo2.png', order: 2 },
    ],
  });
  ...
  const fetchData = async () => {
    try {
      ...
      const keyItem = response.find((item: any) => item.key === 'clients');
      if (keyItem?.data) {
        // Якщо дані – масив, конвертуємо в об'єкт з описом
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/content', {
        ...
        body: JSON.stringify({ key: 'clients', data }),
      });
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-[#1a3c34]">👥 Наші замовники</h1>
        <button
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="Наприклад: Нам довіряють провідні компанії"
            />
          </div>

          <p className="text-sm text-gray-500">Список компаній-замовників (відображаються на головній)</p>
